...
export async function getStaticProps({ params }: Props2): Promise<{ props: Props1 }> {
  const post = matter(ReadService.readFile({ dataPath: `data/tips/content/${params.slug}.md` }));
  const meta = ReadService.readFile({ dataPath: `data/tips/meta/${params.slug}.json` });
  delete post.orig;
